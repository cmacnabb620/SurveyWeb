...
class AdminUserController extends Controller {
    ...
    public function index(){
    	$admin_users= \EnvatoUser::user_type_list(1);
    	/*dd($project_managers);*/
    	if(Auth::user()->user_id == 1){
    		return view('Admin.Users.AdminUser.manage_admin_user',compact('admin_users'));
    	}else{
   	...
   	public function addAdminUser(){
   		if(Auth::user()->user_id == 1){
    		return view('Admin.Users.AdminUser.add_admin_user');
    	}else{
    		return redirect()->to('admin/dashboard');
    	}
    }
        ...
        if ($validator->fails())
        {
         return Response::json(array('success' => false,'errors' => $validator->getMessageBag()->toArray()), 400);
        	...
        	//tag_bridge table store start
        	 ...
        	 $tagbridge_table_store->tag_tag_id=1;
        	 ...
        	 $user_table_insert->user_type_id=1;
    ...
    public function viewDetails($userid){
        ...
        if(count($user_id) == 0){
           ...
           $admin_user= \EnvatoUser::edit_user($user_id,1);
            $user_types=UserType::all();
            if(Auth::user()->user_id == 1){
    		  return view('Admin/Users/AdminUser/view_admin_user',compact('user_types','admin_user'));
	    	}else{
	    		return redirect()->to('admin/dashboard');
	    	}
    ...
    public function editDetails($userid){
        ...
        if(count($user_id) == 0){
            ...
            if(Auth::user()->user_id == 1){
    		  return view('Admin/Users/AdminUser/edit_admin_user',compact('user_types','admin_user'));
	    	}else{
	    		return redirect()->to('admin/dashboard');
	    	}
        }
    ...
    public function deleteAdminUser($userid){
       ...
       $user_record=User::where('user_id',$userid)->first();
        if(!empty($user_record)){
            ...
            return redirect()->to('/admin/admin_users')->with('message', 'Admin User Deleted Successfully');
            }catch (Exception $e) {
