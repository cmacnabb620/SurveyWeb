...
<div id="content" class="span10">
  <ul class="breadcrumb">
    <li>
      <i class="icon-cog"></i>
      <a href="{{ url('quality_user/settings') }}">Settings</a>
    </li>
  </ul>
      ...
      <div class="row-fluid sortable">
        <div class="box span10 offset1">
          ...
          <!--   <form class="form-horizontal" method="POST" id="myform" files= 'true'> -->
            <form class="form-horizontal" action="{{url('quality_user/update_user')}}" method="POST" id="myform" enctype="multipart/form-data">
              <input type="hidden" name="_token" value="{{ csrf_token() }}">
              <input type="hidden" name="user_type" value="{{$qc_user_profile->user_type_id}}">
              <input type="hidden" name="user_id" value="{{$qc_user_profile->main_user_id}}">
              <input type="hidden" name="address_id" value="{{$qc_user_profile->address_table_id}}">
              <input type="hidden" name="contact_id" value="{{$qc_user_profile->contact_id}}">
              <input type="hidden" name="tag_user_id" value="{{$qc_user_profile->tag_user_id}}">
              <!-- ****Profile Image Upload:Start********* -->
              <div class="row-fluid offset2">
                <div class="span3">
                  <label>Upload Profile Image</label>
                  @if(!empty($qc_user_profile->profile_pic))
                  <img id="output_image" class="img-circle" src="{{asset('crossroads/ProfilePics/QcPics/'.$qc_user_profile->profile_pic)}}" />
                  @else
                ...
                <fieldset>
                  <div class="span6">
                    <div class="control-group">
                      ...
                      <div class="controls">
                        <input type="text" class="form-control" name="fname" id="fname"  placeholder="Enter First Name" value="{{\Crypt::decryptString($qc_user_profile->first_name)}}" maxlength="50">
                      </div>
                    </div>
                    <div class="control-group">
                      <label class="control-label" for="lname"><b>Last Name<span style="color:red">*</span></b></label>
                      <div class="controls">
                        <input type="text" class="form-control" name="lname" id="lname"  placeholder="Enter Last Name" value="{{\Crypt::decryptString($qc_user_profile->last_name)}}" maxlength="50">
                      </div>
                    </div>
                    <div class="control-group">
                      <label class="control-label" for="name"><b>Email<span style="color:red">*</span></b></label>
                      <div class="controls">
                        <input type="email" class="form-control" id="email" name="email" placeholder="Enter Email" value="{{\Crypt::decryptString($qc_user_profile->email)}}" maxlength="70" />
                      </div>
                    </div>
                    <div class="control-group">
                      <label class="control-label" for="name"><b>Phone Number<span style="color:red">*</span></b></label>
                      <div class="controls">
                        <input type="text" class="form-control" id="phone" value="{{\Crypt::decryptString($qc_user_profile->phone)}}" name="phone" placeholder="Enter Phone Number" maxlength="14"/>
                      </div>
                    </div>
                    <div class="control-group">
                      <label class="control-label"><b>Date of Birth<span style="color:red">*</span></b></label>
                      <div class="controls">
                        <input type="text" name="date" id="date" class="form-control datepicker" value="{{\Crypt::decryptString($qc_user_profile->DOB)}}" data-format="mm/dd/yyyy">
                      </div>
                    </div>
                  </div>
                  <div class="span6">
                    ...
                    <div class="control-group">
                      ...
                      <div class="controls">
                        <select data-placeholder="Select State" id="state" name="state">
                          <option value="{{Crypt::decryptString($qc_user_profile->state)}}" selected="selected">{{Crypt::decryptString($qc_user_profile->state)}}</option>
                        </select>
                      </div>
                    </div>
                    <div class="control-group">
                      ...
                      <div class="controls">
                        <input type="text" class="form-control" id="city" name="city" placeholder="Enter City" value="{{\Crypt::decryptString($qc_user_profile->city)}}" autocomplete="off" maxlength="50" required>
                      </div>
                    </div>
                    ...
                    <div class="control-group">
                      ...
                      <div class="controls">
                        <textarea rows="3" cols="50" name="address" id="address" placeholder="Address...">{{\Crypt::decryptString($qc_user_profile->address)}}</textarea>
                      </div>
                    </div>
                  </div>
...
// given the id of the <select> tag as function argument, it inserts <option> tags
  ...
  countryElement.options[0] = new Option('{{Crypt::decryptString($qc_user_profile->country)}}', '{{Crypt::decryptString($qc_user_profile->country)}}');
  ...
  <script>
   function passwordUpdationValidationCheck()
     ...
     //post data
      ...
      {
         var url  = "{{url('quality_user/update_manager_password')}}";
         var old_pwd= $('#old_pwd').val();
         var new_password= $('#new_password').val();
