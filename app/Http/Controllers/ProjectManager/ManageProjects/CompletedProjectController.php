...
class CompletedProjectController extends Controller {
    ...
    public function index(){
        $projects = Project::where('project_manager_id',Auth::user()->user_id)->where('admin_posted', '!=', NULL)->where('project_status', 5 )->get()->all();
         //dd($projects);
         $data=[];
         if(!empty($projects)){
      ...
      return view('ProjectManager.ManageProjects.CompletedProjects.manage_completed_projects',compact('project_data'));
    }
    ...
    public function projectInfo($projectid){
      ...
      return view('ProjectManager.ManageProjects.CompletedProjects.detail_view_completed_project',compact('data','project','languages','survey_types','language_ids','survey_type_ids','wk_dates_week_interval_main'));
    }
