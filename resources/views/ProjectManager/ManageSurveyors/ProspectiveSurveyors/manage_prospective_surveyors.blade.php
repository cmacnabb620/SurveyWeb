						...
						<tr>
							...
							<td class="center">
								<abbr title="Surveyor Information">
									<a class="btn btn-primary" href="{{url('project_manager/prospective_surveyor_info/'.Hashids::encode($surveyor->main_user_id).'/'.Hashids::encode($surveyor->user_type_id))}}">
										<i class="halflings-icon white zoom-in"></i>
									</a>
								</abbr>
								@if($surveyor->active == 0)
								<abbr title="Status Inactive">
									<a class="btn btn-danger">
										<i class="halflings-icon white remove"></i>
									</a>
								</abbr>
								@else
								<abbr title="Status Active">
