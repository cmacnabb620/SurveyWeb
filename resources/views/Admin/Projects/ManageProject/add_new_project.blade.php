  ...
  <div class="row-fluid sortable">
    <div class="box span9">
      <div class="box-header" data-original-title>
        ...
        <span>Add New Project</span></h2>
        <div class="box-icon">
          <a href="#" class="btn-minimize"><i class="halflings-icon white chevron-up"></i></a>
        </div>
      </div>
      <div class="box-content">
        <form class="form-horizontal" method="POST" id="addNewProject">
            ...
            <div class="row-fluid">
              <div class="span10 offset1">
                <div class="control-group">
                  ...
                  <div class="controls">
                    <input type="text" class="form-control" id="project_name" name="project_name" placeholder="Project Name" maxlength="50" required>
                  </div>
                </div>
                <div class="control-group">
                  ...
                  <div class="controls">
                    <select data-placeholder="Select Project Manager" name="project_manager_id" id="project_manager_id" data-rel="chosen" required>
                      ...
                      <option value="{{$project_manager->main_user_id}}">{{(\Crypt::decryptString($project_manager->first_name))}}&nbsp;{{(\Crypt::decryptString($project_manager->last_name))}}</option>
                      @endforeach
                      @else
                      <option disabled selected>Select Project Manager</option>
                      @endif
                 ...
                 <div class="control-group">
                  ...
                  <div class="controls">
                    <select data-placeholder="Select Client" data-rel="chosen" name="client_id" id="client_id">
                      ...
                      <option value="{{$client->client_id}}">{{(\Crypt::decryptString($client->name))}}</option>
                      @endforeach
                    </select>
                ...
                <div class="control-group">
                  ...
                  <div class="controls">
                    <select data-placeholder="Select Frequecy Report" name="report_frequency_id" id="report_frequency_id" data-rel="chosen" >
                      ...
                      <option value="{{$rep_freq->report_freq_id}}">{{$rep_freq->report_frequency}}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
                <div class="control-group">
                  ...
                  <div class="controls">
                    <select data-placeholder="Select Status" name="status_id" id="status_id" data-rel="chosen" required>
                      ...
                      <option value="{{$project_status->status_id}}">{{$project_status->status}}</option>
                      @endforeach
                    </select>
                  </div>
                </div>
...
// given the id of the <select> tag as function argument, it inserts <option> tags
  ...
  {
  var url   = "{{url('admin/submit_project_details')}}";
  var csrf  = $('#_token').val();
  var formdata = $('#addNewProject').serialize();
  $.ajax({
  url: url,
  type: 'POST',
  data:formdata,
  ...
  toastr.options.timeOut = 1500; // 1.5s
  toastr.success('Project Stored Successfully.');
  location.reload();
  return false;
  }
